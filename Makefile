...
POGRASS_SRC:=rosuda/pograss/PoGraSS.java rosuda/pograss/PoGraSSimage.java rosuda/pograss/PoGraSSPS.java rosuda/pograss/PoGraSSPDF.java rosuda/pograss/PoGraSSmeta.java rosuda/pograss/PoGraSSgraphics.java
...
JGR_SRC:=$(wildcard rosuda/JGR/*.java)  $(wildcard rosuda/JGR/browser/*.java) $(wildcard rosuda/JGR/layout/*.java) $(wildcard rosuda/JGR/toolkit/*.java) $(wildcard rosuda/JGR/util/*.java) $(wildcard rosuda/JGR/rhelp/*.java) $(wildcard rosuda/JGR/robjects/*.java) $(wildcard rosuda/JGR/editor/*.java)
...
deducer.jar: JGR.jar javaGD.jar iplots.jar MRJstubs.jar $(DEDUCER_SRC) 
	...
	cp -r rosuda/projects/deducer/templates .
...
doc: $(IBASE_SRC) $(KLIMT_SRC) $(PLUGINS_SRC) $(JRCLIENT_SRC) $(JGR_SRC) $(IPLOTS_SRC) $(IWIDGETS_SRC) $(JRI_SRC) $(JAVAGD_SRC) $(RENGINE_SRC) $(RENGINE_RSERVE_SRC) $(RENGINE_JRI_SRC) $(ICUSTOM_SRC) $(DEDUCER_SRC) 
	rm -rf JavaDoc
	mkdir JavaDoc
	javadoc -d JavaDoc -author -version -breakiterator -link $(JAPIURL) $^
